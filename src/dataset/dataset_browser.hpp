...
struct DatasetValidationResult {
  ...
  std::string timestamp_epoch;
  bool cameras_present = false;
  bool lidar_present = false;
  bool lidar_imu_present = false;
  bool gps_rtk_present = false;
  bool rover_rtcm_present = false;
  bool base_rtcm_present = false;
  bool time_sync_present = false;
  uint64_t gps_rtk_navigation_samples = 0;
  uint64_t rover_rtcm_batches = 0;
  uint64_t rover_rtcm_bytes = 0;
  uint64_t rover_rtcm_agent_dropped_bytes = 0;
  uint64_t base_rtcm_batches = 0;
  uint64_t base_rtcm_bytes = 0;
  uint64_t time_sync_samples = 0;
  uint64_t time_sync_provider_transitions = 0;
