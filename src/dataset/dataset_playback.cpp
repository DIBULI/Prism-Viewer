...
bool loadDatasetPlaybackData(
  ...
  *data = {};
  ...
  try {
    ...
    if (std::filesystem::exists(root / "gps_rtk.csv"))
      throw std::runtime_error("Retired raw/smoothed RTK datasets are not supported");
  ...
  try {
    ...
    for (size_t offset = 0; offset < bytes.size();
      ...
      if (batch.model == 3u) {
        ...
        point.line = bytes[offset + 14u];
        const uint8_t flags = bytes[offset + 15u];
        if (flags > 1u || point.line >= 4u || (!flags && point.line))
          throw std::runtime_error("invalid Livox line metadata");
        point.line_valid = flags == 1u;
      }
