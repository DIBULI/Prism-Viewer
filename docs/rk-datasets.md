...
通过 Viewer 当前已连接的 USB 设备下载，不经过 HTTP 或 80 端口。
SDK 仅传输原始格式文件；ROS bag 转换属于 Viewer 的本机功能。
需要包含文件传输扩展的新版 Agent/SDK 1.2.0；旧的同版本二进制可能尚未提供该接口。
...
4. 确认 Viewer 已通过 USB 连接设备，点击刷新；不需要填写 IP、端口或 Web 密码。
...
需要 USB 连接但不需要启动采集，不会修改相机参数，不会删除 RK 原文件。
保存目录会记住；输出目录冲突时拒绝覆盖。
...
- 复用当前 USB 连接和 SDK Client，不需要网络，不另开争用设备的会话。
- RK 采集中拒绝下载；Viewer 不会自动停止设备或争抢采集所有权。
- 下载、写盘、bag 导出在工作线程，支持取消。
- 使用 64 位文件偏移、最长 256 KiB 分块，校验 CRC32、大小、文件版本和前后清单。
  文件版本是变化检测标记，不是 SHA-256；原始内容不转换。
- 失败/取消保留 `.partial-唯一后缀`，完整下载后才改成正式目录。目前不自动续传。
- 原始数据与 bag 都需要电脑磁盘空间，转换失败时已下载原始目录仍保留。
- USB 访问遵循设备现有握手和物理连接边界，不读取浏览器 Cookie，不改变 Web 认证设置。
