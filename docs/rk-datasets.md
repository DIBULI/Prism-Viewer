...
当前 Viewer 的此下载窗口尚未实现 Web 登录。若设备已启用网页密码，
请在浏览器登录设备并下载数据集，解压后通过 Viewer 的“本地数据集”打开，
再导出 ROS bag。无需、也不建议为了此窗口关闭设备认证。
...
- 此窗口只支持无需登录的 Web 服务，无法沿用浏览器的登录 Cookie；
  设备启用认证时采用上面的浏览器下载流程。仅限可信局域网，不应开放公网。
