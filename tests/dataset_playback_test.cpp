...
int main() {
  ...
  try {
    ...
    // Deliberately not index % 4: playback must preserve stored provenance.
    lidar_bytes[14] = 3u; lidar_bytes[15] = 1u;
    ...
    writeText(root / "gps_rtk.csv", "retired navigation data\n");
    DatasetPlaybackData retired;
    require(!loadDatasetPlaybackData(root, {}, &retired, &error) &&
                error.find("Retired raw/smoothed") != std::string::npos,
            "retired RTK dataset must be rejected");
    std::filesystem::remove(root / "gps_rtk.csv");
    ...
    require(points.size() == 2u && points[0].x_mm == 1000 &&
                ...
                points[0].line == 3u && points[0].line_valid &&
                points[1].line == 0u && !points[1].line_valid,
            "LiDAR point payload was decoded incorrectly");

    lidar_bytes[15] = 2;
    writeBytes(root / "lidar-data-0000.bin", lidar_bytes);
    require(!loadDatasetLidarPoints(data.lidar_batches.front(), &points, &error) &&
                points.empty(), "invalid line metadata accepted");
    lidar_bytes[15] = 1;
    writeBytes(root / "lidar-data-0000.bin", lidar_bytes);
